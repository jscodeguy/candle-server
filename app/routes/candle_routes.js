...
// pull in Mongoose model for candles
...
// { candle: { title: '', text: 'foo' } } -> { candle: { text: 'foo' } }
...
router.get('/candles',  (req, res, next) => {
	Candle.find()
		.then((candles) => {
			...
			return candles.map((candle) => candle.toObject())
		})
		// respond with status 200 and JSON of the candles
		.then((candles) => res.status(200).json({ candles: candles }))
...
router.patch('/candles/:id', removeBlanks, (req, res, next) => {
	...
	Candle.findById(req.params.id)
		...
		.then((candle) => {
			...
			return candle.updateOne(req.body.candle)
		})
